...
import type { HudSnapshot } from './hud-types.js';
...
export const createHudDrawableManager = (layer: Container): HudDrawableManager => {
  ...
  let presentation: HudPresentationSnapshot = {
    levelText: '', timerText: '', strategyText: '', riskText: '', primaryText: '', recordText: '', centerText: '',
  };
  ...
  const update = (snapshot: HudSnapshot, layout: HudLayout, theme: EnvironmentTheme, quality: QualityPreset): void => {
    ...
    placeText(strategyText, layout.topCenter, inset);
    placeText(riskText, layout.topRight, inset);
    placeText(primaryText, layout.bottomLeft, inset);
    placeText(recordText, layout.bottomRight, inset);

    centerPanel.visible = center.length > 0;
    centerText.visible = center.length > 0;
    if (center.length > 0) {
      ...
      centerText.style.wordWrap = true;
      centerText.style.wordWrapWidth = layout.event.width - inset * 2;
    } else {
      centerPanel.clear();
    }
  };
  ...
  return {
    update,
    getResourceCounts: () => ({ containers: 1, graphics: 6, texts: 7 }),
...
const formatTarget = (value: number, unit: string): string => {
  ...
  if (unit === 'score') return Math.round(value).toLocaleString('en-US');
...
const centerCopy = (snapshot: HudSnapshot): string => {
  switch (snapshot.run.lifecycle) {
    ...
    case 'restart-countdown':
      return `RESTARTING IN ${formatCountdown(snapshot.run.countdownTicksRemaining, snapshot.run.tickDurationMs)}`;
...
const centerAccent = (snapshot: HudSnapshot, theme: EnvironmentTheme): string => {
  if (snapshot.run.lifecycle === 'paused') return '#FFFFFF';
  if (snapshot.run.lifecycle === 'summary') return '#FFD34D';
  if (snapshot.run.lifecycle === 'restart-countdown') return theme.portal;
